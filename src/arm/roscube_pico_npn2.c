...
#define PLATFORM_NAME "ROSCUBE-PICO-NPN2"
...
static mraa_result_t mraa_roscube_get_pin_index(mraa_board_t* board, char* name, int* pin_index)
{
    ...
    syslog(LOG_CRIT, "ROSCUBE PICO NPN2: Failed to find pin name %s", name);
...
mraa_board_t* mraa_roscube_pico_npn2()
{
    ...
    b->phy_pin_count = MRAA_ROSCUBE_PICO_NPN2_PINCOUNT;
    b->gpio_count = MRAA_ROSCUBE_GPIOCOUNT;
    b->chardev_capable = 0;

    b->pins = (mraa_pininfo_t*) malloc(sizeof(mraa_pininfo_t) * MRAA_ROSCUBE_PICO_NPN2_PINCOUNT);
    if (b->pins == NULL) 
    {
        goto error;
    }
    ...
    syslog(LOG_NOTICE, "ROSCube Pico NPN2: base1 %d \n", base1);
...
error:
    syslog(LOG_CRIT, "ROSCUBE PICO NPN2: Platform failed to initialise");
    free(b);
    close(_fd);
    return NULL;
}
