...
static mraa_result_t mraa_roscube_get_pin_index(mraa_board_t* board, char* name, int* pin_index)
{
    ...
    syslog(LOG_CRIT, "RQX-58G: Failed to find pin name %s", name);
...
mraa_board_t* mraa_roscube_x_58g()
{
    int i2c_bus_num;
    ...
    syslog(LOG_NOTICE, "RQX-58G: base1 %d.\n", base1);
    ...
    mraa_roscube_set_pininfo(b, 51, "FPGA_TRIG1",     (mraa_pincapabilities_t){ 1, 1, 0, 0, 0, 0, 0, 0 }, 408);
    mraa_roscube_set_pininfo(b, 52, "FPGA_TRIG2",     (mraa_pincapabilities_t){ 1, 1, 0, 0, 0, 0, 0, 0 }, 350);
    mraa_roscube_set_pininfo(b, 53, "FPGA_TRIG3",     (mraa_pincapabilities_t){ 1, 1, 0, 0, 0, 0, 0, 0 }, 446);
    mraa_roscube_set_pininfo(b, 54, "FPGA_TRIG4",     (mraa_pincapabilities_t){ 1, 1, 0, 0, 0, 0, 0, 0 }, 445);
...
error:
    syslog(LOG_CRIT, "RQX-58G: Platform failed to initialise");
    free(b);
    close(_fd);
    return NULL;
}
